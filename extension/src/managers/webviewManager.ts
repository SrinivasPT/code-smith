...
export class WebviewManager implements WebviewManagerInterface {
	...
	constructor(private context: vscode.ExtensionContext, private onMessage: (message: WebviewMessage) => void) {}

	createWebviewPanel(): vscode.WebviewPanel {
		this.panel = vscode.window.createWebviewPanel("codesmith", "CodeSmith", vscode.ViewColumn.One, {
			enableScripts: true,
		});
	...
	private setupWebviewContent(): void {
		...
		const scriptUri = this.panel.webview.asWebviewUri(vscode.Uri.file(path.join(assetsPath, jsFile ?? "index.js")));
